/**
 * @module services/reportFormatter
 * @description Centralized, deterministic plain-text Amharic report assembly engine.
 * Strictly adheres to Master Technical Specification Section 3:
 * - 0 markdown characters (#, *, _, `, [])
 * - Strict Ethiopian date DD-MM-YY
 * - 24-hour clock formatting HH:mm
 * - Standard Amharic bullet markers (' - ')
 * - Single blank line delimiters ('\n\n')
 * - Mandatory no_issue and empty comments linguistic fallbacks
 */
import { formatEthiopianReportHeaderDate } from '../utils/ethiopianDate.js';
...
/**
 * Deterministically renders a report data object into locked plain-text Amharic.
 *
 * @param {object} report - Report data object or populated Mongoose document.
 * @param {string|Date} report.date - Calendar date.
 * @param {string} report.branchName - Primary branch name snapshot.
 * @param {string} report.supervisorName - Supervisor full name snapshot.
 * @param {string} report.clockIn - Workday start time (HH:mm).
 * @param {string} report.clockOut - Workday departure time (HH:mm).
...
export const renderReportText = (report) => {
  ...
  const dateFormatted = report.date ? formatEthiopianReportHeaderDate(report.date) : '';
  ...
  let branchLine = '';
  if (visits.length > 0) {
    // Multi-branch visit: collect names in itinerary order
    const branchNames = visits.map((v) => v.branchName || (v.branch && v.branch.name) || '').filter(Boolean);
    // Ensure primary branch name is in list if not already
  ...
  const clockInTime = (report.clockIn || '').trim();
  ...
  // 5. MULTI-BRANCH VISIT TIMELINE INTERVALS (if visits exist)
  if (visits.length > 0) {
    // Sort visits chronologically by clockIn
    const sortedVisits = [...visits].sort((a, b) => (a.clockIn || '').localeCompare(b.clockIn || ''));
  ...
  if (activityItems.length > 0) {
    ...
    // Default single activity placeholder if supervisor provided none
  ...
  // Check if any issue has status 'no_issue' or if array is completely empty
  const hasNoIssueStatus = rawIssues.some((iss) => iss && typeof iss === 'object' && iss.status === 'no_issue');
